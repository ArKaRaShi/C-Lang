...
cache_t *init_cache(int size) {
    ...
    for (int itr = 0; itr < size; itr++) {
        (cache->table + itr)->data = 1; 
        (cache->table + itr)->mem_addr = -1;
    }
    return cache;
}

void copy_data(int addr, memory_t *memory, cache_t *cache) {
...
void get_data(int addr, memory_t *memory, cache_t *cache) {
    ...
    if ((cache->table + cache_index)->mem_addr == -1) {
        ...
        printf("Data: %d\n", *(memory + addr));
        copy_data(addr, memory, cache);
    } else if ((cache->table + cache_index)->data != *(memory + addr)) {
